                ...
                var createRowItems_1 = function (item, contentWidth, startX, index) {
                    ...
                    var getXY = function (type, layoutCfg) {
                        ...
                        switch (type) {
                            ...
                            case 'title':
                                x = startX + offsetX;
                                y = item.name ? paddingArray[0] + rowHeight[1] + constants_1.defaultMargin : (size[1] - fontSize) / 2;
                                break;
                            default:
                                break;
                        }
                    ...
                    keys.forEach(function (key, keyIndex) {
                        ...
                        var shapeStyle = (0, utils_1.getStyle)(labelStyle, cfg, group, getKey_1(key));
                        if (key === 'icon' && item[key]) {
                            iconWidth = shapeStyle.width || 32;
                        }
                        var keyShape = group.addShape(isIcon ? 'image' : 'text', {
                            attrs: __assign(__assign(__assign(__assign({ textBaseline: 'top', textAlign: autoWidth ? 'start' : 'center' }, getXY(key, shapeStyle)), { text: item[key], img: item[key] }), (isIcon ? defaultIconStyle : constants_1.defaultLabelStyle)), shapeStyle),
