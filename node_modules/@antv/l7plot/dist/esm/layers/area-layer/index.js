...
var AreaLayer = /** @class */ (function (_super) {
    ...
    function AreaLayer(options) {
         ...
         * 图层类型
         */
        _this.type = PlotLayer.LayerType.AreaLayer;
        /**
         * 图层是否具有交互属性
         */
        ...
        _this.onSelectHandle = function (event) {
            ...
            var feature = event.feature, featureId = event.featureId;
            var selectData = clone(_this.selectData);
            ...
            if (index === -1) {
                ...
                _this.emit('select', feature, clone(selectData));
            }
            else {
                ...
                _this.emit('unselect', unselectFeature, clone(selectData));
            }
        ...
        var defaultState = getDefaultState(_this.options.state);
        _this.name = name ? name : uniqueId(_this.type);
        _this.layer = new PolygonLayer(__assign(__assign({}, config), { name: _this.name }));
        _this.strokeLayer = new LineLayer({ name: 'strokeLayer', visible: visible, zIndex: zIndex, minZoom: minZoom, maxZoom: maxZoom });
        _this.highlightLayer = new LineLayer({
            name: 'highlightLayer',
        ...
        _this.selectFillLayer = new PolygonLayer({
        ...
        _this.selectStrokeLayer = new LineLayer({
    ...
    AreaLayer.prototype.setSelectLayerSource = function (selectData) {
        ...
        if (this.selectData.length === selectData.length &&
            isEqual(this.selectData.map(function (_a) {
    ...
    AreaLayer.prototype.update = function (options) {
        ...
        if (this.options.visible) {
            ...
            var defaultState = getDefaultState(this.options.state);
    ...
    AreaLayer.prototype.updateHighlightLayer = function () {
        var defaultState = getDefaultState(this.options.state);
        var lasetDefaultState = getDefaultState(this.lastOptions.state);
