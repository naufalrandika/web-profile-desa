...
var Choropleth = /** @class */ (function (_super) {
    ...
    function Choropleth() {
         ...
         * 图表类型
         */
        _this.type = Plot.PlotType.Choropleth;
        /**
         * 国界数据
         */
        ...
        _this.onDrillDownHander = function (event) {
            ...
            var _c = _this.drillSteps[depth], level = _c.level, _d = _c.granularity, granularity = _d === void 0 ? DEFAULT_AREA_GRANULARITY[level] : _d, drillConfig = __rest(_c, ["level", "granularity"]);
            var downParams = {
                nativeEvent: true,
            ...
            var callback = function (config) {
                ...
                var mergeConfig = deepAssign({}, drillConfig, config);
        ...
        _this.onDrillUpHander = function () {
            ...
            var callback = function (config) {
                ...
                var mergeConfig = deepAssign({}, to.config, config);
                _this.changeView(view, mergeConfig).then(function (drillData) {
                    if (drillData) {
    ...
    Choropleth.prototype.update = function (options) {
        ...
        if (options.drill &&
            ...
            !isEqual((_a = this.lastOptions.drill) === null || _a === void 0 ? void 0 : _a.steps, (_b = this.options.drill) === null || _b === void 0 ? void 0 : _b.steps)) {
            this.drillReset();
            this.initDrillEvent();
        }
        ...
        if (options.viewLevel && !isEqual(this.lastOptions.viewLevel, this.options.viewLevel)) {
            ...
            this.getDistrictData(geoData).then(function () {
                var _a = _this.options.source, data = _a.data, sourceConfig = __rest(_a, ["data"]);
            ...
            if (options.source && !isEqual(this.lastOptions.source, this.options.source)) {
                var _d = this.options.source, data = _d.data, sourceConfig = __rest(_d, ["data"]);
    ...
    Choropleth.prototype.parserSourceConfig = function (source) {
        var joinData = source.data, joinBy = source.joinBy, sourceCFG = __rest(source, ["data", "joinBy"]);
    ...
    Choropleth.prototype.createSource = function () {
        ...
        var source = new Source(data, sourceCFG);
        return source;
    };
    /**
     * 更新: 更新数据
    ...
    Choropleth.prototype.changeData = function (data, cfg) {
        var _this = this;
        this.options.source = deepAssign({}, this.options.source, __assign({ data: data }, cfg));
    ...
    Choropleth.prototype.createLayers = function (source) {
        this.fillAreaLayer = new AreaLayer(__assign({ name: 'fillAreaLayer', source: source }, pick(this.options, AreaLayer.LayerOptionsKeys)));
        var layerGroup = new LayerGroup([this.fillAreaLayer]);
    ...
    Choropleth.prototype.createCountryBoundaryLayer = function (data, plotConfig) {
        var _a = createCountryBoundaryLayer(data, plotConfig), chinaBoundaryLayer = _a.chinaBoundaryLayer, chinaHkmBoundaryLayer = _a.chinaHkmBoundaryLayer, chinaDisputeBoundaryLayer = _a.chinaDisputeBoundaryLayer;
    ...
    Choropleth.prototype.createLabelLayer = function (source, label) {
        ...
        var textLayer = new TextLayer(__assign({ name: 'labelLayer', source: {
                data: data,
    ...
    Choropleth.prototype.updateLayers = function (options) {
        var _this = this;
        var fillAreaLayerConfig = pick(options, AreaLayer.LayerOptionsKeys);
    ...
    Choropleth.prototype.initDrillEvent = function () {
        ...
        if (!this.drillStacks.length) {
            var _g = this.options.viewLevel, level = _g.level, adcode = _g.adcode, _h = _g.granularity, granularity = _h === void 0 ? DEFAULT_AREA_GRANULARITY[level] : _h;
            var config = getDrillStepDefaultConfig(this.options);
    ...
    Choropleth.prototype.getCurrentDrillSteps = function () {
        var steps = this.drillStacks.map(function (item) { return pick(item, ['level', 'adcode', 'granularity']); });
        return steps;
    };
    /**
     * 实现 legend 配置项
            ...
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0:
                        ...
                        cacheArea = getCacheArea(fileName);
                        if (cacheArea)
                            return [2 /*return*/, cacheArea];
                        _a = getGeoAreaConfig(this.options.geoArea), url = _a.url, type = _a.type, extension = _a.extension;
        ...
        return __awaiter(this, void 0, void 0, function () {
            ...
            return __generator(this, function (_c) {
                switch (_c.label) {
                    ...
                    case 2:
                        _b = __read.apply(void 0, [_c.sent(), 1]), this.chinaBoundaryData = _b[0];
                        return [3 /*break*/, 4];
                    case 3:
                        err_1 = _c.sent();
    ...
    Choropleth.prototype.getDistrictData = function (geoData) {
        return __awaiter(this, void 0, void 0, function () {
            ...
            return __generator(this, function (_d) {
                switch (_d.label) {
                    case 0:
                        _a = this.options.viewLevel, level = _a.level, adcode = _a.adcode, _b = _a.granularity, granularity = _b === void 0 ? DEFAULT_AREA_GRANULARITY[level] : _b;
    ...
    Choropleth.prototype.drillDown = function (view, config) {
        var _this = this;
        if (config === void 0) { config = {}; }
        var from = this.drillStacks.slice(-1)[0];
        var level = view.level, adcode = view.adcode, granularity = view.granularity;
        var downParams = {
            nativeEvent: false,
            from: { level: from.level, adcode: from.adcode, granularity: from.granularity },
            to: { level: level, adcode: adcode, granularity: granularity },
        };
        // TODO: remove view
    ...
    Choropleth.prototype.drillUp = function (config, level) {
        ...
        var _a = this.drillStacks[stacksIndex], drillConfig = _a.config, view = __rest(_a, ["config"]);
        var mergeConfig = deepAssign({}, drillConfig, config);
        var from = this.drillStacks[this.drillStacks.length - 1];
        var upParams = {
            nativeEvent: false,
            from: { level: from.level, adcode: from.adcode, granularity: from.granularity },
            to: { level: view.level, adcode: view.adcode, granularity: view.granularity },
        };
        this.changeView(view, mergeConfig).then(function (drillData) {
            if (drillData) {
                if (isCustomUp) {
                    _this.drillStacks.splice(customUpStackIndex + 1);
                }
                else {
                    _this.drillStacks.pop();
                }
    ...
    Choropleth.prototype.changeView = function (view, config) {
        ...
        return __awaiter(this, void 0, void 0, function () {
            ...
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0:
                        level = view.level, adcode = view.adcode, _a = view.granularity, granularity = _a === void 0 ? DEFAULT_AREA_GRANULARITY[level] : _a;
                        return [4 /*yield*/, this.fetchData(level, adcode, granularity)];
                    case 1:
                        geoData = _b.sent();
                        if (!geoData.features.length)
                            return [2 /*return*/];
                        mergeConfig = deepAssign({}, getDrillStepDefaultConfig(this.options), config, {
     ...
     * 行政数据地址
     */
    Choropleth.GeoAreaUrl = GEO_AREA_URL;
    return Choropleth;
}(Plot));
export { Choropleth };
