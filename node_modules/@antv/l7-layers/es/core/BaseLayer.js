...
import { isEqual, isFunction, isNumber, isObject, isPlainObject, isUndefined } from 'lodash';
  ...
  _createClass(BaseLayer, [{
    ...
    value: function encodeStyle(options) {
      ...
      Object.keys(options).forEach(function (key) {
        if (
        // 需要数据映射
        [_this6.enableShaderEncodeStyles].concat(_toConsumableArray(_this6.enableDataEncodeStyles)).includes(key) && isPlainObject(options[key]) && (options[key].field || options[key].value) && !isEqual(_this6.encodeStyleAttribute[key], options[key]) // 防止计算属性重复计算
        ) {
