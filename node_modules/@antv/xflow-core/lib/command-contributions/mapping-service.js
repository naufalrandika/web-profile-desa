...
var GraphMappingHelper = /** @class */ (function () {
    function GraphMappingHelper() {
        ...
        this.buildNodeMapping = function (currentNode, nextNode) {
            var currentNodeId = currentNode.id || currentNode.originalId;
        ...
        this.createEdgeBetweenNodes = function (edgeConfig) {
            var id = edgeConfig.id, source = edgeConfig.source, sourcePortId = edgeConfig.sourcePortId, target = edgeConfig.target, targetPortId = edgeConfig.targetPortId, rest = tslib_1.__rest(edgeConfig, ["id", "source", "sourcePortId", "target", "targetPortId"]);
            return tslib_1.__assign(tslib_1.__assign({}, rest), { id: undefined, originalId: id, source: _this.nodeMappingRecord.get(source), target: _this.nodeMappingRecord.get(target), sourcePortId: _this.portMappingRecord.get(sourcePortId), targetPortId: _this.portMappingRecord.get(targetPortId), sourcePort: _this.portMappingRecord.get(sourcePortId), targetPort: _this.portMappingRecord.get(targetPortId) });
        };
        // 更新GroupNode的NodeId
